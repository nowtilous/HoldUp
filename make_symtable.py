...
def get_symbol_table_from_file(filename):
	...
	if (found != len(symbol_list)):
		print "    [WARNING] %s symbol find fail, [%d/%d]" % (filename, found, len(symbol_list))
		for i in range(0, len(symbol_list) - found):
			symbol_table.append(["", 0x0])
	else:
...
if __name__ == "__main__":
	...
	fp_output.write("/* Do not edit this file! It is generated automatically by make_symbol.py */\n\n")
	fp_output.write("char* global_kernel_version[] = \n{\n");
	for item in kernel_version:
		fp_output.write('\t"%s",\n' % item)
	fp_output.write("};\n\n");

	# Write symbol table.
	fp_output.write("struct symbol_table_struct global_symbol_table_array[] =\n{\n");
	table_index = 0
